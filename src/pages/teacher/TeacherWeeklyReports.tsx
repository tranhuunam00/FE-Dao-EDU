...
export const TeacherWeeklyReports: React.FC = () => {
  const [classes, setClasses] = useState<Array<{ id: string; className: string; classCode: string }>>([]);
  ...
  // 1. Tải danh sách các lớp giáo viên phụ trách
  useEffect(() => {
    api
      .get('/classes')
      .then(({ data }) => {
      ...
      >
        <div>
          ...
          <Text style={{ fontSize: 13, color: 'var(--text-secondary, #6b7280)' }}>
            Theo dõi chất lượng học tập và chỉ số SQI của học sinh các lớp phụ trách
          </Text>
        </div>
