...
import { GraduationCap, BarChart2, BookOpen, Calendar, RefreshCw } from 'lucide-react';
...
export const StudentDashboard: React.FC = () => {
  ...
  if (loading) return <div style={{ color: 'var(--text-secondary)' }}>Đang tải dữ liệu...</div>;
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '4px' }}>Theo dõi kết quả học tập và lịch thi của bạn</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px', marginBottom: '32px' }}>
        ...
        <div className="glass-panel" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <div>
            <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Kỳ thi sắp tới</div>
            <div style={{ fontSize: '1.75rem', fontWeight: 700, color: '#fff', marginTop: '4px' }}>{data?.upcomingExams.length ?? 0}</div>
          </div>
        </div>
      </div>
